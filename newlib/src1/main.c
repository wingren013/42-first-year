...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/09/23 08:08:31 by smifsud           #+#    #+#             */
/*   Updated: 2016/09/28 19:45:59 by smifsud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	str_split_test(void)
{
	...
	t = ft_strsplit("$$$$$$split$$$$$this$for$$$ me$$!$$$$$", '$');
	while (*t)
	{
		ft_putendl(*t);
		t++;
...
int		main(void)
{
	...
	ft_putendl(strstr("MZIRIBMZIRIBMZE123", "MZIRIBMZE"));
	ft_putendl(ft_strstr("MZIRIBMZIRIBMZE123", "MZIRIBMZE"));
	ft_putendl("--------------------------------------------------------------");
	char	*teststr1 = (char*)malloc(sizeof(char) * 3);
	teststr1[0] = 'a';
	teststr1[1] = 'b';
	teststr1[2] = 0;
	ft_putendl(ft_strnstr("MZIRIBMZIRIBMZE123", "MZIRIBMZE", ft_strlen("MZIRIBMZE")));
	teststr1 = ft_reversestr(teststr1);
	ft_putendl(teststr1);
	memcpy_test();
	memccpy_test();
	memchr_test();
...
//	size_t_test();
	...
	char	*ptr1 = "";
	char	*ptr2 = "lkljkhkgjg";
	printf("%p\n", ptr1);
	printf("%p\n", ptr2);
	printf("%p\n", strstr(ptr1, ptr2));
	printf("%p\n", ft_strstr(ptr1, ptr2));
	ft_putendl("fuck this it's not crashing");
	ft_putnbr('s');
	ft_putnbr('n');
	ft_putnbr('o');
	ft_putnbr('w');
	...
	ft_putnbr('s'+'n'+'o'+'w');
	ft_putendl(0);
	char **r = ft_strsplit("", '\65');
	for (int i = 0; i < 100; i++)
	{
		ft_strsplit("", '1');
		ft_strsplit("ladida", 'a');
		ft_strsplit("$$$$$$are $$$$$you $$$$not ent$ertained$$$$$$$", '$');
	}
	char *teststr = ft_strnew(70000);
	memset(teststr, '9', 69999);
	teststr[1000] = '8';
	teststr[150] = '8';
	teststr[700] = '8';
	teststr[5000] = '8';
